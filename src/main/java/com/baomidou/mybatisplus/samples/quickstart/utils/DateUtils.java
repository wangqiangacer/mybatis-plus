...
public final class DateUtils {
    ...
    private final static ThreadLocal<DateFormat> FORMATER_DATE = new ThreadLocal<>();

    private final static ThreadLocal<DateFormat> FORMATER_DATE_TIME = new ThreadLocal<>();

    private final static ThreadLocal<DateFormat> FORMATER_DATE_TIME_NO_SEPARATOR = new ThreadLocal<>();

    private final static ThreadLocal<DateFormat> FORMATER_DATE_TIME_NO_SECOND = new ThreadLocal<>();
    ...
    public static DateFormat getFormaterDate() {
        DateFormat formater = FORMATER_DATE.get();
        if (formater == null) {
            formater = new SimpleDateFormat(DATE_FORMAT_YYYY_MM_DD);
            FORMATER_DATE.set(formater);
        }
        return formater;
    }
    ...
    public static DateFormat getFormaterDateTimer() {
        DateFormat formater = FORMATER_DATE_TIME.get();
        if (formater == null) {
            formater = new SimpleDateFormat(DATE_FORMAT_YYYY_MM_DD_HH_MM_SS);
            FORMATER_DATE_TIME.set(formater);
        }
        return formater;
    }
    ...
    public static DateFormat getFormaterDateTimeNoSeparator() {
        DateFormat formater = FORMATER_DATE_TIME_NO_SEPARATOR.get();
        if (formater == null) {
            formater = new SimpleDateFormat(DATE_FORMAT_YYYYMMDDHHMMSS);
            FORMATER_DATE_TIME_NO_SEPARATOR.set(formater);
        }
        return formater;
    }
    ...
    public static DateFormat getFormaterDateTimeNoSecond() {
        ...
        if (formater == null) {
            ...
            FORMATER_DATE_TIME_NO_SECOND.set(formater);
        }
        return formater;
    }
    ...
    public static Date getDateForDateAndTime(String dateOld, String timeOld) {
        if (StringUtils.isEmpty(dateOld) || dateOld.length() != 8) {
            return new Date();
        }
        if (StringUtils.isEmpty(timeOld) || timeOld.length() < 8 || timeOld.length() > 9) {
            timeOld = "000000000";
        }
        if (timeOld.length() == 8) {
            timeOld = ("0" + timeOld).substring(0, (timeOld.length() - 3));
        }
        return DateUtils.parseDate((dateOld + timeOld), "yyyyMMddHHmmss");
    }

    public static void main(String[] args) {
        String timeOld = "82333000";
        System.out.println("0" + timeOld.substring(0, (timeOld.length() - 3)));
    }
}
